...
class Employee {
public:
    Employee(const string, const string, Date&, Date&);
...
void Employee::setInformation(string firstName, string lastName, Date &birthDate, Date &hireDate) {
    this -> firstName = firstName;
    this -> lastName = lastName;
    this -> birthDate = birthDate;
    this -> hireDate = hireDate;
...
const Employee& Employee::getMostFaith(const Employee employees[], int n) {
    Employee mostFaith = employees[0];
    for (int i = 0; i < n; i++)
        if (employees[i].getHireDate() > mostFaith.getHireDate())
        mostFaith = employees[i];
    return mostFaith;
}
...
int main()
{
    ...
    Employee employee[5] = {Employee("Gary", "Tom", Date(1978, 2, 5), Date(2000, 3, 4)),
                ...
                Employee("Ramon", "Buck", (1968, 5, 6), (1992, 3, 5)),
                Employee("Noah", "Ethan", (1996, 5, 4), (2013, 3, 5)),
                Employee("Tyson", "Oedipus", (1992, 11, 23), (2004, 4, 2))};
    return 0;
}
